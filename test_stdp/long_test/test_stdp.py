...
neuron_pre = nest.Create("parrot_neuron", N)
neuron_post = nest.Create("parrot_neuron", N)
...
nest.Connect(sg, neuron_pre, "all_to_all", syn_dict_sg_pre)
syn_dict_sg_post={"weight":1.0, "delay":sg_delay_m+Dt_offset/2.0}
nest.Connect(sg, neuron_post, "all_to_all", syn_dict_sg_post)
...
for j in range(N):
    for i in range(N):
        ...
        syn_dict_stdp={"weight":weight_stdp, "delay":delay_pre, \
                       ...
                       "den_delay":delay_post, "receptor_type": 1}
        
        nest.Connect(neuron_pre[i], neuron_post[j], "one_to_one", syn_dict_stdp)
...
for j in range(N):
    for i in range(N):
        conn_id = nest.GetConnections(neuron_pre[i], neuron_post[j])
